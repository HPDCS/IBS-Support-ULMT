#ifndef __IBS_DISASSEMBLE__
#define __IBS_DISASSEMBLE__
...
static long get_call_operand(unsigned char *byte, unsigned int count)
{
    if (byte[0] == 0xe8 && count == 5) /* call imm32 */
    {
        return  (((unsigned int) byte[1]) | (((unsigned int) byte[2]) << 8) |
                    (((unsigned int) byte[3]) << 16) | (((unsigned int) byte[4]) << 24));
    }
    else
    {
...
static unsigned int disassemble(unsigned char *bytes, unsigned int max, int offset, char *output) {
